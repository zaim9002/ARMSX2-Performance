...
// q11 likewise holds the EE FPU's unscale constant, below, so the range yields
// q10 and q12-q15 in practice).
...
// d11 = 2^kEeFprScaleExp (kEeFprUnscaleBits, EeFpuFormat.h), parked for the
// whole JIT session by _DynGen_EnterRecompiledCode alongside s8/s9 and read by
// iFPUd-arm64.cpp wherever a slot becomes the value it denotes. Emitted in
// every clamp mode: the dispatcher outlives a clamp-mode change.
//
// Same contract as NEON_RESERVED_FPU_MAX/MIN: the lower 64 bits of d8-d15 are
// callee-saved, so a parked constant needs no compile-time liveness tracking,
// no invalidation at C-call seams and no re-materialization.
//
// It costs one slot out of the callee-saved allocator range above, which
// _allocGPRtoNEONreg draws from alone. Three is the floor, what a 3-operand MMI
// op needs at once; the fourth keeps an FPU multiply-accumulate's Fd, Fs, Ft and
